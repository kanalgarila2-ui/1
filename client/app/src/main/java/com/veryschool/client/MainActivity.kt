    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            VerySchoolTheme {
                ...
                val chats by vm.chats.collectAsStateWithLifecycle(emptyList())
                ...
                LaunchedEffect(Unit) {
                    vm.uiEvent.collectLatest { event ->
                        when (event) {
                            is UiEvent.Error -> Toast.makeText(
                                this@MainActivity,
                                event.msg,
                                Toast.LENGTH_LONG
                            ).show()
                            is UiEvent.Success -> Toast.makeText(
                                this@MainActivity,
                                event.msg,
                                Toast.LENGTH_SHORT
                            ).show()
                ...
                LaunchedEffect(authState) {
                    when (authState) {
                        AuthState.NeedServer -> navController.navigate(Routes.CONNECT) {
                            popUpTo(navController.graph.startDestinationId) { inclusive = true }
                        }
                        AuthState.NeedAuth -> navController.navigate(Routes.AUTH) {
                            popUpTo(navController.graph.startDestinationId) { inclusive = true }
                        }
                        AuthState.Authenticated -> navController.navigate(Routes.CHAT_LIST) {
                            popUpTo(navController.graph.startDestinationId) { inclusive = true }
                        }
                        AuthState.Unknown -> { /* ничего не делаем */ }
                    }
                }
