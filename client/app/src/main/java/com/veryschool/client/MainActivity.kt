            ...
            VerySchoolTheme {
                ...
                NavHost(navController = navController, startDestination = Routes.CONNECT) {
                    ...
                    composable(Routes.AUTH) {
                        AuthScreen(onLogin = { u, p, ph -> vm.login(u, p, ph) }, onRegister = { u, p, dn, ph -> vm.register(u, p, dn, ph) })
                    }
                    composable(Routes.CHAT_LIST) {
                        ChatListScreen(
                            chats = chats, users = users, currentUserId = currentUserId,
                            displayName = displayName, avatarBase64 = avatar, connected = connected,
                            onChatClick = { chat -> currentChat = chat; vm.openChat(chat.id); navController.navigate(Routes.chat(chat.id)) },
                            onNewDm = { user ->
                                val existing = chats.firstOrNull { c -> !c.isGroup && c.members.contains(user.id) && c.members.contains(currentUserId) }
                                if (existing != null) { currentChat = existing; vm.openChat(existing.id); navController.navigate(Routes.chat(existing.id)) }
                    ...
                    composable(Routes.CHAT) { backStack ->
                        ...
                        ChatScreen(
                            chatName = chat?.name ?: chatId, chatAvatar = chat?.avatarBase64 ?: "",
                            isGroup = chat?.isGroup ?: false, messages = messages,
                    ...
                    composable(Routes.PROFILE) {
                        ProfileScreen(userId = currentUserId, username = username, displayName = displayName, avatarBase64 = avatar, serverUrl = serverUrl,
                            onBack = { navController.popBackStack() },
                            onSave = { dn, uri -> vm.updateProfile(dn, uri, this@MainActivity) },
                            onChangePassword = { old, new -> vm.changePassword(old, new) },
                            onLogout = { vm.logout(); navController.navigate(Routes.AUTH) { popUpTo(0) { inclusive = true } } })
                    }
                    composable(Routes.USER_PROFILE) { backStack ->
                        ...
                        UserProfileScreen(
                            userId = userId, displayName = user?.displayName ?: userId,
                            username = user?.username ?: "", avatarBase64 = user?.avatarBase64 ?: "",
                            online = user?.online ?: false,
                            onBack = { navController.popBackStack() },
                            onSendMessage = {
                                val existing = chats.firstOrNull { c -> !c.isGroup && c.members.contains(userId) && c.members.contains(currentUserId) }
                                if (existing != null) { currentChat = existing; vm.openChat(existing.id); navController.navigate(Routes.chat(existing.id)) { popUpTo(Routes.CHAT_LIST) } }
