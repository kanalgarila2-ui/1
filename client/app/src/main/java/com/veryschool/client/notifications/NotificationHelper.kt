...
object Ch { const val MESSAGES = "vs_messages"; const val SYSTEM = "vs_system"; const val ERRORS = "vs_errors" }
...
class NotificationHelper(private val ctx: Context) {
    ...
    private fun pi() = PendingIntent.getActivity(ctx, 0,
    ...
    fun showMessage(sender: String, text: String) {
        try {
            NotificationManagerCompat.from(ctx).notify(nid(),
                NotificationCompat.Builder(ctx, Ch.MESSAGES)
                    .setSmallIcon(android.R.drawable.ic_dialog_email).setContentTitle(sender).setContentText(text)
                    .setStyle(NotificationCompat.BigTextStyle().bigText(text))
                    .setPriority(NotificationCompat.PRIORITY_HIGH).setAutoCancel(true)
                    .setContentIntent(pi()).setColor(Color.rgb(139, 92, 246)).build())
        } catch (_: SecurityException) {}
    }
    fun showBanned(reason: String = "") {
        try {
            NotificationManagerCompat.from(ctx).notify(999,
                NotificationCompat.Builder(ctx, Ch.SYSTEM)
    ...
    fun showFrozen(frozen: Boolean) {
        try {
            NotificationManagerCompat.from(ctx).notify(998,
                NotificationCompat.Builder(ctx, Ch.SYSTEM)
    ...
    fun showSyncError(msg: String) {
        try {
            NotificationManagerCompat.from(ctx).notify(nid(),
                NotificationCompat.Builder(ctx, Ch.ERRORS)
                    .setSmallIcon(android.R.drawable.ic_dialog_alert)
